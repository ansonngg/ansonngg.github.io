...
export default function StartGame(event) {
    ...
    otherElements = document.querySelectorAll('.home-intro-wrapper > *:not(.game-entity-wrapper), #profile-picture');
...
function InitGame(event) {
    ...
    home.style.border = '3px solid #000000';
    home.style['border-radius'] = '10px';
    home.style.overflow = 'hidden';

    if (event == 'entity-0') {
        enemy = document.getElementById('entity-0');
        notEnemy = document.getElementById('entity-1');
        notEnemy.classList.add('down-fade-out');
    } else {
        enemy = document.getElementById('entity-1');
        notEnemy = document.getElementById('entity-0');
        notEnemy.classList.add('up-fade-out');
    }
    ...
    otherElements.forEach((element) => {
        element.classList.add('down-fade-out');
    });
...
function GameLoop() {
    ...
    fixedUpdate = setInterval(() => {
        ...
        if (
            !godMode &&
            (cursorX == -1 ||
                collidingElem.classList.contains('enemy') ||
                collidingElem.classList.contains('bullet') ||
                !home.matches(':hover'))
        ) {
...
async function EndGame() {
    ...
    if (selectedId == 'entity-0') {
        notEnemy.classList.remove('down-fade-out');
        notEnemy.onclick = () => StartGame('entity-1');
    } else {
        notEnemy.classList.remove('up-fade-out');
        notEnemy.onclick = () => StartGame('entity-0');
    }
...
function SpinAttack(timeStamp) {
    ...
    if (elapsed < spinAccelTime) {
        ...
        enemy.style.transform = `translate(${Number(stats[1]) + xDiff}px, ${Number(stats[2]) + yDiff}px) rotate(${
            (Number(stats[3]) + diff * realSpinSpeed) % 360
        }deg)`;
        ...
        enemy.style.transform = `translate(${Number(stats[1]) + xDiff}px, ${Number(stats[2]) + yDiff}px) rotate(${
            (Number(stats[3]) + diff * spinSpeed) % 360
        }deg)`;
    } else if (elapsed < spinAccelTime + spinMaxTime + spinDecelTime) {
        let maxDist =
            ((diff * (spinAccelTime + spinMaxTime + spinDecelTime - elapsed)) / spinDecelTime) * spinMoveSpeed;
        if (magnitude > maxDist) {
            xDiff = (maxDist * xDiff) / magnitude;
            yDiff = (maxDist * yDiff) / magnitude;
        }
    ...
    if (elapsed < 28 * rifleInterval) {
        ...
        if (Math.floor(elapsed / rifleInterval) % 4 == 0) {
            if (temp.length == 0) {
                temp.push(
                    ((Math.atan2(
                        cursorY - (enemyInitY + Number(stats[2]) + enemyHeight / 2),
                        cursorX - (enemyInitX + Number(stats[1]) + enemyWidth / 2)
                    ) *
                        180) /
                        Math.PI +
                        360) %
            ...
            enemy.style.transform = `translate(${stats[1]}px, ${stats[2]}px) rotate(${
                (Number(stats[3]) + (temp[1] * diff) / rifleInterval) % 360
            }deg)`;
        } else if (!bulletShot) {
...
function SpearAttack(timeStamp) {
    ...
    if (elapsed < 20 * spearInterval) {
        ...
        if (Math.floor(elapsed / spearInterval) % 4 == 0) {
            ...
            enemy.style.transform = `translate(${stats[1]}px, ${stats[2]}px) rotate(${
                (Number(stats[3]) + (temp[1] * diff) / spearInterval) % 360
            }deg)`;
            ...
            if (resultX < -enemyWidth / 2) {
                ...
                resultY -=
                    (resultX - (home.clientWidth - enemyWidth / 2)) * Math.tan((Number(stats[3]) * Math.PI) / 180);
            ...
            if (resultY < -enemyInitY - enemyHeight / 2) {
                ...
                resultX -=
                    ...
                    Math.tan((Number(stats[3]) * Math.PI) / 180);
                resultY = -enemyInitY + home.clientHeight - enemyHeight / 2;
            }
